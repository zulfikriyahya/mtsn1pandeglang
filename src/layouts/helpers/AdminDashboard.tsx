...
interface User {
  ...
  email: string;
  picture: string;
  role: string; // Updated to string to handle potential nulls safely
}
...
const AdminDashboard = () => {
  ...
  // Modal Konfirmasi Hapus (General Purpose)
  const [confirmModal, setConfirmModal] = useState<{
    isOpen: boolean;
    ids: number[];
    type: string; // feedback, survey, user
    count: number;
    action?: () => void; // Custom action for user deletion
  }>({ isOpen: false, ids: [], type: "feedback", count: 0 });

  // Modal Status (Sukses/Gagal)
  const [statusModal, setStatusModal] = useState<{
    isOpen: boolean;
    status: "success" | "error";
    title: string;
