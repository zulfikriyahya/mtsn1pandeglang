<?php
// === DEBUG MODE (Hapus/Komentari baris ini jika sudah fix) ===
...
if (!file_exists('lib/fpdf.php')) {
    die("Error: File library 'public/api/lib/fpdf.php' tidak ditemukan. Pastikan Anda sudah mengupload file fpdf.php ke folder tersebut.");
}
...
class PDF extends FPDF
{
    ...
    // Fungsi Aman untuk Menampilkan Gambar Remote (QR)
    function ImageRemote($url, $x, $y, $w, $h)
    {
        // Coba download konten
        ...
        if ($content !== false) {
            ...
            unlink($tmpFile); // Hapus setelah dipakai
        } else {
            // Jika gagal download, tampilkan kotak kosong + teks
            $this->SetXY($x, $y);
            $this->SetFont('Arial', '', 6);
            $this->Cell($w, $h, 'QR Error', 1, 0, 'C');
        }
    }
...
// Pastikan server bisa akses internet. Jika gagal, fungsi ImageRemote akan handle.
